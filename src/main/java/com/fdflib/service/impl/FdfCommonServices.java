...
public abstract class FdfCommonServices {
    ...
    private static <S extends CommonState> void getOrderByPosition(S state) {
        SqlStatement statement = SqlStatement.build().select("id").select("order").where(setForCurrent(state.tid));
        if(state.order == 1) {
            statement.limit(1,1);
        } else {
    ...
    private static <S extends CommonState> void validateOrder(S state) {
        ...
        List<S> orderBetween = SqlStatement.build().select("id").where(whereOrder).where(setForCurrent(state.tid))
